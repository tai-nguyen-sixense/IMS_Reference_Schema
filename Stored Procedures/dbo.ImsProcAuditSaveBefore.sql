...
CREATE PROCEDURE [dbo].[ImsProcAuditSaveBefore] @Id uniqueidentifier, @auditJson nvarchar(max), @UserId uniqueidentifier
...
Declare @returnAuditJson as nvarchar(max);
	...
	BEGIN TRY  
		...
		-- the procedure can be customized : using simple replace
					-- set @returnAuditJson=replace(@auditJson,'"comments":null','"comments":"please add comments"');
