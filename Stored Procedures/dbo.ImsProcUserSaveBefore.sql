...
CREATE PROCEDURE [dbo].[ImsProcUserSaveBefore] @Id uniqueidentifier, @userJson nvarchar(max), @UserId uniqueidentifier
...
Declare @returnUserJson as nvarchar(max);
	...
	BEGIN TRY  
		...
		-- the procedure can be customized : using simple replace
					-- set @returnUserJson=replace(@userJson,'"comments":null','"comments":"please add comments"');
