...
-- Description:	Return allow in json format when user get an taskDocument
-- InPut:		@Id = TaskDocument.Id, @allowJson= default allow in json format, @UserId = Loggged User Id
...
CREATE PROCEDURE [dbo].[ImsProcTaskDocumentAllow] @Id uniqueidentifier,@allowJson nvarchar(max), @UserId uniqueidentifier
...
BEGIN
	...
	BEGIN TRY  
		...
		--{"actOn":true,"edit":true,"delete":true,"copy":true,"check":true,"lockedFields":["name"]}
